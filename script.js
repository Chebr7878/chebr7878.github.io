...
(function(){
  ...
  if (openBtn && closeBtn && drawer) {
    openBtn.addEventListener('click', ()=>{ drawer.classList.add('open'); topbar.classList.add('nav-open'); });
    closeBtn.addEventListener('click', ()=>{ drawer.classList.remove('open'); topbar.classList.remove('nav-open'); });
  }
})();

// Поиск с порогом 3 символа и дебаунсом
(function(){
      ...
      document.querySelectorAll('section').forEach(block=>{
        block.childNodes.forEach(node=>{
          if (node.nodeType === 3 && re.test(node.textContent)){
            ...
            span.innerHTML = node.textContent.replace(re, m=>`<mark class="__hl">${m}</mark>`);
            node.replaceWith(...span.childNodes);
          }
        });
      });
      const first = document.querySelector('mark.__hl');
